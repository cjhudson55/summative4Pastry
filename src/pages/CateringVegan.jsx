...
const CateringVegan = () => {
    ...
    const FFPosts = ({ posts }) => {
        ...
        const mappedPosts = posts.map((post, index) => {
            ...
            if (post.acf.category_type === 'fingerfood' && dietaryRequirement.includes(4)) {
    ...
    const HTPosts = ({ posts }) => {
        ...
        const mappedPosts = posts.map((post, index) => {
            ...
            if (post.acf.category_type === 'hightea' && dietaryRequirement.includes(4)) {
                function getFeaturedImage(post) {
    ...
    const CakePosts = ({ posts }) => {
        ...
        const mappedPosts = posts.map((post, index) => {
            ...
            if (post.acf.category_type === 'cakes' && dietaryRequirement.includes(4)) {
                function getFeaturedImage(post) {
                    if (post && post._embedded && post._embedded['wp:featuredmedia'] && post._embedded['wp:featuredmedia'][0]) {
                        console.log('success');
                        return post._embedded['wp:featuredmedia'][0].source_url;
